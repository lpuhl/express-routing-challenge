  ...
  $('#song-form').on('submit', function () {
    ...
    $.ajax({
      ...
      error: function (response) {
          alert('Please enter a new title and artist!');
      }
    });
  });
  ...
  $.ajax({
    ...
    success: function (songs) {
      ...
      songs.forEach(function (song) {
        $('#song-list').append('<div><p class="title-artist">' + song.title + '-' + song.artist + '</p><p class="date">Date added: ' + song.dateAdded + '</p></div>');
      });
    },
    error: function () {
